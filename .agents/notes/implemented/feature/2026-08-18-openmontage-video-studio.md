...
**Always-expanded full form.** Rejected in favor of a compose-first sheet: the primary path stays brief and specs; upscale, wiki picks, and paste stay under More options so the first viewport does not compete with secondary fields.

**From the harness `openwiki/` tree, bundle a slug/title/summary catalog.** Rejected because the production Workspace is often an OpenMontage checkout whose sandbox cannot read the harness tree; listing that Workspace's own `openwiki/` directory matches the directory the agent will use.
