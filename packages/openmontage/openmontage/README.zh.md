...
| `outputDurationSeconds` | `30` | 工作室默认输出时长（秒）。工作室表单和首条用户消息使用该值；不写入检出 `.env`。 |
| `outputResolution` | `1080p` | 工作室默认生成清晰度：`480p`、`720p`、`1080p` 或 `4k`。与时长一样走 Settings 和首条消息。 |
| `outputUpscaleTo` | （空） | 可选的生成后超分目标：`720p`、`1080p` 或 `4k`，且必须严格高于 `outputResolution`。空表示不超分。 |
| `generationProfile` | `auto` | 工作室生成方案偏好：`auto`、`cost`、`quality` 或 `drama`。写入 settings 与首条用户消息；不切换供应商 SDK。 |
